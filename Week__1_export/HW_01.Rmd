...
```
```
The observed result was attained by taking 1*1,2*2,1*3,2*4,1*5,2*6
```
...
```{r linewidth=80}
a = matrix(c(0,0,0,1)*1:20,nrow =4)
a
```
...
```{r linewidth=80}
z <- sort(union(setdiff(u,v),setdiff(v,u)))
z
sum(is.element(u,z))+sum(is.element(v,z))
```
...
```{r linewidth=80}
as.double(names(table(v))[which.max(table(v))])
```
