...
for (ii in 1:length(name)) {
  counter = 0
  for (jj in 1:nchar(name[ii])){
    counter = counter + 1
  }
  cat("word:", name[ii],"length:",counter,"\n")
}
```
...
```
```
Print shows the entire string, while cat interprets the newline character.
```
...
```
```
Everything is counted, including the comma and the newline.
```
...
```{r linewidth=80}
clinton.lines[grep("Bill",clinton.lines,value=FALSE)]
```
...
```
Thre are 2426 lines.
The last earthquake was record 2017/02/24
and it was magnitude 6.90
```
...
reg.exp <- regexpr(geo.pattern.sp,trial.str.vec.sp)
reg.exp
regmatches(trial.str.vec.sp,reg.exp)
```
...
```{r linewidth=80}
geo.pattern.pair = "(|-)[0-9]{1,3}\\.[0-9]{4} +(|-)[0-9]{1,3}\\.[0-9]{4}"
reg.expr = regexpr(geo.pattern.pair,date.lines)
lat.lon.pairs = regmatches(date.lines,reg.expr)
lat.lon.pairs[1:3]
```
...
```{r linewidth=80}
res <- strsplit(unlist(lat.lon.pairs[1:2]),split = " +")
res = as.vector(res)
new_res = matrix(c(res[[1]],res[[2]]),nrow=2)
t(new_res)
```
