...
char2int = function(x)
{
  ...
  if (!is.element(x,letters) & !is.element(x,LETTERS)) return(NULL)
  if (is.element(x,letters)) return(which(x == letters))
  if (is.element(x,LETTERS)) return(which(x == LETTERS))
}
char2int(1)
char2int('abc')
...
my_factorial = function(x)
{
  ...
  if(as.integer(x) != x | x < 0) return(NULL)
  if(x == 0){return(1)}
  if(x == 1){return(1)}
...
root = function(x,t)
{
  ...
  res = list(sqrt.x = r,n.iter = iter)
  return(res)
}
print_results = function(r,x)
{
  cat('approximated value:',r[['sqrt.x']],'\nreal value:',sqrt(x))
  cat('\nabsolute difference',abs(r[['sqrt.x']]-sqrt(x)))
  cat('\nNumber of iterations',r[['n.iter']],"\n\n")
}
t = 10^-6
r1 = root(11,t)
r2 = root(101,t)
...
print_results(r1,11)
print_results(r2,101)
print_results(r3,1001)
```
