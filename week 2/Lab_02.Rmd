...
logistic(1,2,1,1) # x = x_0 = 1, returns 1, which is 1/2 of 2
```
...
```{r linewidth=80}
logistic = function(x,L=2,x.o=1,k=1)
{
  res = L / (1 + exp(-k*(x-x.o)))
  return(res)
}
...
```
```
They don't, the first two return the same result,
because they are the same function call
with the parameters being specified in different orders.
The third line returns a different value because it calls
logistic(x=2,L=1,x.o=1,k=-1) which is different from
logistic(x=1,L=1,x.o=-1,k=2)
```
...
```{r linewidth=80}
#checks that all function inputs are numeric
logistic = function(x,L=2,x.o=1,k=1)
{
  if (!is.numeric(c(x,L,x.o,k))) return(NULL)
  res = L / (1 + exp(-k*(x-x.o)))
  return(res)
}
```
...
```
```
It is a power function. First line sets
z to be square then takes 3 ^2
third sets to be cubed
then takes 2^3
```
...
```{r linewidth=80}
#takes input x, and returns function that subtracts x from input
z = function(x)
{
  return(function(y){y-x})  
}
...
```
```
h is a function with internal constant y = 2
it has a function i, with interal constant z= 3
and it returns i()
calling h, returns i, which returns c(x,y,z)
and y,z are already determined,
and x is defined environmentally
so it returns 1 2 3
```
