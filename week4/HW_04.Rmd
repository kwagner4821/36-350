...
res <- state.df %>%
  group_by(state.df$region,state.df$division) %>%
  summarize(fifth = quantile(Income,.05),ninetyfifth = quantile(Income,.95),dif = quantile(Income,.95)- quantile(Income,.05)) %>%
  arrange(.,desc(dif))
res
```
...
```{r linewidth=80}
newer.sprint.df %>% separate(.,Birthdate,into = c("other","BirthYear"),sep = 6) %>%filter(.,Year == 2011) %>% arrange(desc(DayOfYear)) %>% select(First.Name,Last.Name,BirthYear) %>% head(.,1)
```
